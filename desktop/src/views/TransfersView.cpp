...
void TransfersView::RenderHeader() {
  ...
  if (activeCount > 0) {
    ...
    ImGui::TextColored(theme_->GetColorVec(ThemeColor::Success),
                       "%d active transfer%s", activeCount,
                       activeCount == 1 ? "" : "s");
  }
}
...
void TransfersView::RenderTransferCard(const TransferInfo &transfer,
                                       int index) {
  ...
  drawList->AddCircleFilled(ImVec2(iconX + 12, iconY + 12), 22,
                            ImGui::ColorConvertFloat4ToU32(ImVec4(
                                iconColor.x, iconColor.y, iconColor.z, 0.15f)));
  ...
  ImGui::TextColored(theme_->GetColorVec(ThemeColor::TextSecondary), "%s",
                     fileName.empty() ? "Preparing..." : fileName.c_str());
  ...
  drawList->AddRectFilled(
      ...
      theme_->GetColor(ThemeColor::SurfaceLight), progressBarHeight * 0.5f);
  ...
  if (progress > 0) {
    ...
    drawList->AddRectFilled(ImVec2(progressBarX, progressBarY),
                            ImVec2(progressBarX + progressBarWidth * progress,
                                   progressBarY + progressBarHeight),
                            progressColor, progressBarHeight * 0.5f);
    ...
    if (transfer.state == TELEPORT_STATE_TRANSFERRING) {
      ...
      glowVec.w = 0.3f;
      drawList->AddRectFilled(
          ImVec2(progressBarX + progressBarWidth * progress - 20,
                 progressBarY - 4),
          ImVec2(progressBarX + progressBarWidth * progress + 5,
                 progressBarY + progressBarHeight + 4),
          ImGui::ColorConvertFloat4ToU32(glowVec), 6.0f);
    }
  }
  ...
  teleport_format_bytes(transfer.bytesTransferred, transferredStr,
                        sizeof(transferredStr));
  teleport_format_bytes(transfer.bytesTotal, totalStr, sizeof(totalStr));
  teleport_format_bytes((uint64_t)transfer.speedBps, speedStr,
                        sizeof(speedStr));

  ImGui::TextColored(theme_->GetColorVec(ThemeColor::TextSecondary), "%s / %s",
                     transferredStr, totalStr);

  if (transfer.state == TELEPORT_STATE_TRANSFERRING) {
    ImGui::SameLine(0, 20);
    ImGui::TextColored(theme_->GetColorVec(ThemeColor::TextPrimary), "%s/s",
                       speedStr);
  ...
  case TELEPORT_STATE_HANDSHAKING:
    ...
    ImGui::TextColored(theme_->GetColorVec(ThemeColor::Warning),
                       "Connecting...");
    break;
  ...
  case TELEPORT_STATE_TRANSFERRING: {
    ...
    if (ImGui::Button(
            (std::string(ICON_PAUSE) + "##Pause" + std::to_string(index))
                .c_str(),
            ImVec2(36, 36))) {
    ...
    if (ImGui::Button(
            (std::string(ICON_CANCEL) + "##Cancel" + std::to_string(index))
                .c_str(),
            ImVec2(36, 36))) {
  ...
  case TELEPORT_STATE_PAUSED: {
    ...
    if (ImGui::Button(
            (std::string(ICON_PLAY) + "##Resume" + std::to_string(index))
                .c_str(),
            ImVec2(36, 36))) {
  ...
  case TELEPORT_STATE_COMPLETE: {
    ImGui::SetCursorScreenPos(ImVec2(controlsX, controlsY));
    ImGui::PushFont(theme_->GetIconFont());
    ImGui::TextColored(theme_->GetColorVec(ThemeColor::Success), "%s",
                       ICON_CHECK);
    ImGui::PopFont();
    ImGui::SameLine(0, 10);
  ...
  case TELEPORT_STATE_CANCELLED: {
    ImGui::SetCursorScreenPos(ImVec2(controlsX, controlsY));
    ImGui::PushFont(theme_->GetIconFont());
    ImGui::TextColored(theme_->GetColorVec(ThemeColor::Error), "%s",
                       ICON_ERROR);
    ImGui::PopFont();
    ImGui::SameLine(0, 10);
    ImGui::TextColored(theme_->GetColorVec(ThemeColor::Error),
                       transfer.state == TELEPORT_STATE_CANCELLED ? "Cancelled"
                                                                  : "Failed");
    break;
  }
...
void TransfersView::RenderEmptyState() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    drawList->AddTriangleFilled(
        ...
        ImVec2(center.x + 20 * scale, center.y - offset * scale), color);
  }

  // Center icon box
  drawList->AddRectFilled(ImVec2(center.x - 35, center.y - 15),
                          ImVec2(center.x + 35, center.y + 25),
                          theme_->GetColor(ThemeColor::SurfaceLight), 8.0f);

  ImGui::PushFont(theme_->GetIconFont());
  ImVec2 iconSize = ImGui::CalcTextSize(ICON_UPLOAD);
  ImGui::SetCursorScreenPos(ImVec2(center.x - 18, center.y - 5));
  ImGui::TextColored(theme_->GetColorVec(ThemeColor::Primary), "%s",
                     ICON_UPLOAD);
  ImGui::SameLine(0, 4);
  ...
  ImGui::SetCursorScreenPos(
      ImVec2(center.x - textSize.x * 0.5f, center.y + 50));
  ImGui::TextColored(theme_->GetColorVec(ThemeColor::TextSecondary), "%s",
                     text);
  ...
  ImGui::SetCursorScreenPos(
      ImVec2(center.x - hintSize.x * 0.5f, center.y + 75));
