...
void Theme::InitDarkColors() {
  ...
  colors_[ThemeColor::Accent] =
      ImVec4(0.133f, 0.827f, 0.933f, 1.0f); // #22D3EE (Cyan)

  // Surfaces - Dark with glass effect
  colors_[ThemeColor::Background] =
      ImVec4(0.067f, 0.067f, 0.090f, 0.95f); // Near black
  colors_[ThemeColor::Surface] =
      ImVec4(0.094f, 0.094f, 0.106f, 0.85f); // #18181B
  colors_[ThemeColor::SurfaceLight] =
      ImVec4(0.153f, 0.153f, 0.165f, 0.80f); // #27272A
  colors_[ThemeColor::SidebarTop] =
      ImVec4(0.078f, 0.078f, 0.094f, 0.95f); // Dark gradient top
  colors_[ThemeColor::SidebarBottom] =
      ImVec4(0.055f, 0.055f, 0.071f, 0.98f); // Dark gradient bottom
  colors_[ThemeColor::Card] =
      ImVec4(0.110f, 0.110f, 0.125f, 0.75f); // Card background
  colors_[ThemeColor::CardHover] =
      ImVec4(0.140f, 0.140f, 0.160f, 0.85f); // Card hover

  // Text
  colors_[ThemeColor::TextPrimary] =
      ImVec4(0.980f, 0.980f, 0.980f, 1.0f); // #FAFAFA
  colors_[ThemeColor::TextSecondary] =
      ImVec4(0.631f, 0.631f, 0.667f, 1.0f); // #A1A1AA
  colors_[ThemeColor::TextDisabled] =
      ImVec4(0.400f, 0.400f, 0.430f, 1.0f); // #666670

  // States
  colors_[ThemeColor::Success] =
      ImVec4(0.063f, 0.725f, 0.506f, 1.0f); // #10B981
  colors_[ThemeColor::Warning] =
      ImVec4(0.961f, 0.620f, 0.043f, 1.0f);                          // #F59E0B
...
void Theme::LoadFonts(ImGuiIO &io) {
  // DO NOT call io.Fonts->Clear() - it can leave ImGui in invalid state if
  // fonts fail to load
...
#else
  // Linux fonts - try common locations
  const char *bodyPath = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf";
  const char *boldPath = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf";
  const char *iconPath = nullptr; // No equivalent icon font on Linux

  // Try Ubuntu fonts if DejaVu not found
  FILE *f = fopen(bodyPath, "r");
  if (!f) {
    bodyPath = "/usr/share/fonts/truetype/ubuntu/Ubuntu-R.ttf";
    boldPath = "/usr/share/fonts/truetype/ubuntu/Ubuntu-B.ttf";
  } else {
    fclose(f);
  }
#endif
  ...
  ImFont *body = io.Fonts->AddFontFromFileTTF(bodyPath, 16.0f);
  if (body) {
    bodyFont_ = body;
  }

  // Try to load bold font for headings
  ImFont *bold = io.Fonts->AddFontFromFileTTF(boldPath, 26.0f);
  if (bold) {
    headingFont_ = bold;
  }

  // Try to load smaller font for labels
  ImFont *small = io.Fonts->AddFontFromFileTTF(bodyPath, 12.0f);
  if (small) {
    smallFont_ = small;
  }

  // Try to load icon font (Windows only for now)
  if (iconPath) {
    ImFont *icons = io.Fonts->AddFontFromFileTTF(iconPath, 18.0f);
    if (icons) {
      iconFont_ = icons;
    }
  }
...
void DrawGradientRect(ImDrawList *drawList, ImVec2 min, ImVec2 max,
  ...
  drawList->AddRectFilledMultiColor(min, max, topColor, topColor, bottomColor,
                                    bottomColor);

  // If rounding is needed, we apply it via clipping
  if (rounding > 0) {
    // For proper rounded gradient, we'd need a shader - this is a limitation of
    // ImGui Using simple rounded rect overlay as workaround
  }
}

void DrawGlow(ImDrawList *drawList, ImVec2 center, float radius, ImU32 color,
              float intensity) {
