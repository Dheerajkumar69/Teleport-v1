...
class IncrementalSHA256 {
  ...
  update(chunk) {
    // Store reference directly — callers never reuse the buffer (it comes from
    // a streaming reader that discards it after each read).  Avoiding the copy
    // halves peak memory for large files.
    this.chunks.push(chunk);
  ...
  async hex() {
    ...
    if (
      ...
      try {
        ...
        console.warn(
          "[IncrementalSHA256] SubtleCrypto failed, using pure-JS fallback:",
          e.message,
        );
      }
    }
...
function selectChunkSize(fileSize) {
  // Tiered chunk sizing — balance throughput (larger chunks) against
  // resumability and memory pressure (smaller chunks).
  if (fileSize < 100 * 1024 * 1024) return 16 * 1024;   // <100 MB  → 16 KB
  if (fileSize < 500 * 1024 * 1024) return 256 * 1024;  // 100–500 MB → 256 KB
  if (fileSize < 2 * 1024 * 1024 * 1024) return 1024 * 1024; // 500 MB–2 GB → 1 MB
  return 4 * 1024 * 1024;                               // ≥2 GB → 4 MB
}
// ============================================================================
...
const ErrorMessages = {
  ...
  [ErrorCodes.SIGNALING_FAILED]:
    "Signaling failed - unable to reach signaling server",
  [ErrorCodes.ICE_FAILED]:
    "ICE connection failed - no compatible NAT traversal",
  ...
  [ErrorCodes.TRANSFER_CORRUPTED]:
    "Transfer corrupted - data integrity check failed",
  [ErrorCodes.HASH_MISMATCH]:
    "Hash mismatch - file content verification failed",
  ...
  [ErrorCodes.RELAY_CONNECTION_FAILED]:
    "Relay connection failed - server unavailable",
  [ErrorCodes.RELAY_TIMEOUT]: "Relay timeout - server not responding",
  [ErrorCodes.STREAMING_WRITE_FAILED]:
    "Streaming write failed - disk I/O error",
  ...
  [ErrorCodes.UNSUPPORTED_FEATURE]:
    "Unsupported feature - peer does not support",

  [ErrorCodes.NO_PEER_AVAILABLE]:
    "No peer available - no connections established",
  [ErrorCodes.TRANSFER_REJECTED]: "Transfer rejected by peer",
  [ErrorCodes.AUTHENTICATION_FAILED]:
    "Authentication failed - fingerprint mismatch",
};
...
class TeleportWebRTC {
  constructor() {
    ...
    this.deviceName =
      this.loadSetting("teleport-device-name") || this.generateDeviceName();
    this.peers = new Map();
    ...
    this._fileResponseHandlers = new Map(); // peerId -> handler(accepted, fromId) for relay responses
    ...
    this.maxBandwidth =
      parseInt(this.loadSetting("teleport-bandwidth-limit")) || 0;
    ...
    this.onPeerVerificationNeeded = null;
    ...
    // Pre-stored File System API handles (set from user gesture, consumed async).
    // Maps filename -> FileSystemFileHandle
    this._pendingFileHandles = new Map();
  ...
  // handleError is defined below as a unified implementation handling both
  // handleError(ErrorCode, context) and handleError(message, errorObj) signatures.
  ...
  cleanupTransfer(transferId) {
    ...
    if (transfer.writer) {
      transfer.writer
        .abort?.()
        .catch((e) => console.warn("Writer abort failed:", e));
    }

    // Clear relay data
    const relayTransfer = this.relayIncoming.get(transferId);
    if (relayTransfer) {
      relayTransfer.writer
        ?.abort?.()
        .catch((e) => console.warn("Relay writer abort failed:", e));
  ...
  cleanupPeer(peerId) {
    ...
    // Close peer connection (peer IS the RTCPeerConnection, not a wrapper)
      ...
      case "transfer-cancel":
        if (this.onTransferError) {
          this.onTransferError({
            ...
            peerId: data.peerId,
            fileName: data.fileName,
            error: data.reason || "Transfer cancelled in another tab",
            crossTab: true,
          });
        }
        break;
      case "transfer-error":
        if (this.onTransferError) {
          this.onTransferError({
            ...
            peerId: data.peerId,
            fileName: data.fileName,
            error: data.error || "Transfer error in another tab",
            crossTab: true,
          });
        }
        break;
  ...
  setBandwidthLimit(bytesPerSecond) {
    ...
    // Reset token bucket: if limit increased, allow immediate burst up to new limit;
    // if decreased, cap tokens to new limit so we don't send a burst at old rate.
    const now = Date.now();
    const elapsed = (now - this.lastThrottleTick) / 1000;
    this.lastThrottleTick = now;
    if (bytesPerSecond > 0) {
      const refill = bytesPerSecond * elapsed;
      this.bandwidthTokens = Math.min(bytesPerSecond, this.bandwidthTokens + refill);
    } else {
      this.bandwidthTokens = 0;
    }
  ...
  getDefaultSignalingServers() {
    ...
    const runtimeConfigured = Array.isArray(window.TELEPORT_SIGNALING_SERVERS)
      ? window.TELEPORT_SIGNALING_SERVERS.map((url) =>
          this.normalizeWebSocketUrl(url),
        ).filter(Boolean)
      : [];
    ...
    // Empty hostname means file:// protocol — treat as localhost
    if (host === "" || host === "localhost" || host === "127.0.0.1") {
      return [
        this.normalizeWebSocketUrl("ws://localhost:3000"),
        this.normalizeWebSocketUrl("ws://127.0.0.1:3000"),
        this.normalizeWebSocketUrl(
    ...
    return [
      ...
      this.normalizeWebSocketUrl("wss://teleport-signaling.onrender.com"),
      this.normalizeWebSocketUrl(
        "wss://teleport-signaling-backup.onrender.com",
      ),
    ].filter(Boolean);
  }
  ...
  setConnectionMode(mode) {
    ...
    try {
      localStorage.setItem("teleport-connection-mode", mode);
    } catch {}
  ...
  async getRankedSignalingServers(explicitServer = null) {
    ...
    const probes = await Promise.all(
      uniqueCandidates.map((url) => this.probeSignalingServer(url)),
    );
  ...
  async fetchTurnCredentials() {
    ...
    for (const wsUrl of candidates) {
      try {
        ...
        if (Array.isArray(config.iceServers) && config.iceServers.length > 0) {
          ...
          console.log(
            "[Teleport] TURN credentials refreshed from server",
            wsUrl,
          );
          return;
        }
    ...
    console.warn(
      "[Teleport] Could not fetch TURN credentials from any signaling endpoint, using fallback TURN list",
    );
  }
  ...
  startKeepAlive() {
    ...
    console.log(
      "[Teleport] Keep-alive started (every 14 min) for",
      this.serverUrl,
    );
  }
  ...
  sanitizeFilename(filename) {
    ...
    if (name.length === 0) {
      throw new Error(
        `Filename reduced to empty after sanitization: ${filename}`,
      );
    }
  ...
  initResumeDB() {
    ...
    try {
      ...
      req.onsuccess = (e) => {
        this.resumeDb = e.target.result;
      };
      req.onerror = () => {
        console.warn("[Resume] IndexedDB unavailable");
      };
    } catch (e) {
  ...
  async throttle(bytes) {
    ...
    this.bandwidthTokens = Math.min(
      this.maxBandwidth,
      this.bandwidthTokens + refill,
    );
    ...
    if (this.bandwidthTokens < bytes) {
      ...
      this.bandwidthTokens = Math.min(
        this.maxBandwidth,
        this.bandwidthTokens + refillAfterWait,
      );
    }
  ...
  clearTransferHistory() {
    try {
      localStorage.removeItem("teleport-transfer-history");
    } catch (e) {}
  }
  ...
  async connect(serverUrl = null) {
    if (this._connecting) {
      console.warn("[Teleport] connect() called while already connecting — rejecting concurrent call");
      return;
    }
    ...
    try {
      ...
      for (const candidate of rankedServers) {
        try {
          ...
          console.warn(
            "[Teleport] Signaling candidate failed:",
            candidate,
            e.message || e,
          );
        }
      ...
      const finalError = lastError || new Error("No signaling servers reachable");
      if (this.onConnectionFailed) {
        this.onConnectionFailed(finalError);
      }
      throw finalError;
    } finally {
      this._connecting = false;
  ...
  connectToSignalingServer(serverUrl) {
    return new Promise((resolve, reject) => {
      ...
      const attemptTimeout = Math.min(
        ...
        this.SIGNALING_ATTEMPT_TIMEOUT,
      );
      let settled = false;
      let joined = false;
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          console.warn(
            "[Teleport] Dropping invalid signaling payload:",
            e.message,
          );
          return;
        }
          ...
          this.generateFingerprint()
            .then(async () => {
              ...
              if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
                rejectOnce(
                  new Error("WebSocket closed before join handshake completed"),
                );
                return;
              }
  ...
  async generateFingerprint() {
    try {
      ...
      this.peerFingerprint = hashArray
        .map((b) => b.toString(16).padStart(2, "0"))
        .join("")
        .substring(0, 16)
        .toUpperCase();
      ...
      this.peerFingerprint = Array.from(
    ...
    try {
      ...
      if (!trusted) {
        ...
        return {
          valid: true,
          trustLevel: "pending",
          reason: "First contact - waiting for manual verification",
        };
      }

      // Verify fingerprint matches
      if (
        trusted.fingerprint.toUpperCase() !== incomingFingerprint.toUpperCase()
      ) {
        console.error(
          `[Security] Fingerprint mismatch for peer ${peerId}: expected ${trusted.fingerprint}, got ${incomingFingerprint}`,
        );
        return {
          valid: false,
          trustLevel: "rejected",
          reason: "Fingerprint mismatch - possible MITM attack",
        };
      }
  ...
  async addTrustedDevice(peerId, fingerprint, publicKey) {
    ...
    try {
      ...
      return {
        success: true,
        message: `Peer ${peerId} added to trusted devices`,
      };
    } catch (e) {
  ...
  async removeTrustedDevice(peerId) {
    try {
      ...
      return {
        success: true,
        message: `Peer ${peerId} removed from trusted devices`,
      };
    } catch (e) {
  ...
  async verifyPeerSignature(announcement, publicKeyPem) {
    /**
     * Verify peer identity using TOFU (Trust On First Use) model.
     *
     * Strategy:
     * 1. If no signature/publicKey: allow but flag as unverified
     * 2. If known peer: verify fingerprint matches stored record
     * 3. If unknown peer: auto-trust, store for future verification
     * 4. If fingerprint mismatch with trusted record: reject (MITM)
     */
    const { peerId, fingerprint } = announcement;

    if (!fingerprint) {
      return {
        valid: true,
        trustLevel: "unverified",
        reason: "Peer provided no fingerprint",
      };
    }

    // Delegate to the existing fingerprint validation infrastructure
    const validation = await this.validatePeerFingerprint(
      peerId,
      fingerprint,
    ...
    if (validation.valid) {
      // Store the public key for this peer if provided
      if (publicKeyPem && peerId) {
    ...
    return {
      valid: false,
      trustLevel: "rejected",
      reason: validation.reason || "Fingerprint verification failed",
    };
  }
  ...
  async computeFileSha256(file) {
    ...
    const digestHex = await hasher.hex();
  ...
  // Unified handleError — handles both (code, context) and (message, error) call signatures
  handleError(codeOrMessage, contextOrError) {
    // Detect which calling convention is being used
    if (ErrorMessages && ErrorMessages[codeOrMessage]) {
      // Called as handleError(ErrorCodes.X, { transferId, ... })
      const code = codeOrMessage;
      const context = contextOrError || {};
      const message = ErrorMessages[code] || "Unknown error";
      const fullError = { code, message, context, timestamp: Date.now() };
      console.error(`[TeleportError] ${code}: ${message}`, context);
      if (this.onError) {
        try {
          this.onError(fullError);
        } catch (e) {
          console.warn("[TeleportError] onError callback threw:", e);
        }
      }
      if (this.onTransferError && context.transferId) {
        try {
          ...
          console.warn("[TeleportError] onTransferError callback threw:", e);
        }
        // Broadcast error to other tabs
        this.broadcastEvent("transfer-error", {
          transferId: context.transferId,
          error: message,
        });
      }
      ...
      // Called as handleError('some message', errorObj)
  ...
  disconnect() {
    ...
    this.isConnected = false;
    // Safely close only RTCPeerConnection objects (not signaling peer metadata)
    ...
    this.peers.clear();
    // Close each DataChannel before clearing references to avoid leaks.
    this.dataChannels.forEach(function (dc) {
      if (dc && typeof dc.close === "function" && dc.readyState !== "closed") {
        try {
          dc.close();
        } catch (e) {}
      }
        ...
        try {
            switch (message.type) {
                ...
                case 'relay-start': {
                    ...
                    if (typeof message.sha256 !== 'undefined' && message.sha256 !== null && !expectedSha256) {
                    ...
                    let safeRelayFilename;
                    try {
                        safeRelayFilename = this.sanitizeFilename(message.filename);
                    } catch (e) {
                        console.warn('[Relay] Filename rejected:', e.message, '- using fallback');
                        safeRelayFilename = 'received_file';
                    }
                    ...
                    if (useStreaming && this.useFileSystemAPI) {
                        try {
                            writer = new StreamingFileWriter(safeRelayFilename, message.size, {
                                useFileSystemAPI: true
                            });

                            // Use resolveFileHandle: checks pre-stored handle first,
                            // falls back to showSaveFilePicker (works in user gesture).
                            const handle = await this.resolveFileHandle(safeRelayFilename, message.mimeType);
                            if (handle) {
                                await writer.initFileSystemAPI(handle);
                                console.log('[Relay] Initialized File System API streaming writer for', safeRelayFilename);
                            } else {
                                console.log('[Relay] No file handle available, buffering in memory');
                                writer = null;
                            }
                        } catch (e) {
                ...
                case 'relay-chunk': {
                    ...
                    if (transfer) {
                        ...
                        // Decrypt if transfer is encrypted
                        let chunkBytes = bytes;
                        if (transfer.encrypted && message.iv && transfer.from) {
                            try {
                                const ivBytes = Uint8Array.from(atob(message.iv), c => c.charCodeAt(0));
                                chunkBytes = new Uint8Array(await this.decryptData(transfer.from, bytes, ivBytes));
                            } catch (e) {
                                console.error('[Relay] Decryption failed:', e.message);
                                break;
                            }
                        }
                        ...
                        if (transfer.writer) {
                            try {
                                await transfer.writer.write(chunkBytes);
                            } catch (e) {
                        ...
                        if (transfer.hasher) {
                            transfer.hasher.update(chunkBytes);
                        }
                        ...
                        if (Number.isFinite(transfer.size) && transfer.receivedBytes > transfer.size) {
                            ...
                            this.sendRelayVerificationAck(
                                ...
                                false,
                                'size-overflow',
                                transfer.hasher ? await transfer.hasher.hex() : null
                            );
                ...
                case 'relay-end': {
                    ...
                    if (transfer) {
                        ...
                        const actualSha256 = transfer.hasher ? await transfer.hasher.hex() : null;
                        // Use sha256 from relay-end if relay-start didn't have it (incremental hash path).
                        const expectedSha256 = transfer.expectedSha256 || this.normalizeSha256(message.sha256);
                        const sizeMismatch = Number.isFinite(transfer.size) && totalSize !== transfer.size;
                        const hashMismatch = !!expectedSha256 && actualSha256 !== expectedSha256;
                ...
                case 'relay-resume-request': {
                    // Desktop → Browser: receiver has N bytes already, continue from resumeOffset.
                    // If we have an active outgoing transfer for this transferId, update its offset.
                    // The sendFileViaRelay loop is paused on a Promise; we signal it via a flag.
                    const { transferId, resumeOffset } = message;
                    if (typeof transferId !== 'string' || typeof resumeOffset !== 'number') break;
                    if (resumeOffset < 0 || !Number.isFinite(resumeOffset)) break;

                    console.log(`[Relay] Resume request for ${transferId}: desktop has ${resumeOffset} bytes`);

                    const state = this.activeTransfers.get(transferId);
                    if (state) {
                        // Store the acked offset so the send loop can skip ahead on next iteration
                        state.resumeOffset     = resumeOffset;
                        state.resumeRequested  = true;
                    } else {
                        // Transfer not yet started or already finished — store as pending hint
                        this._pendingResumeHints = this._pendingResumeHints || new Map();
                        this._pendingResumeHints.set(transferId, resumeOffset);
                    }
                    break;
                }

                case 'relay-reconnect-hint': {
                    // Desktop → Browser (on reconnect): desktop still has a partial temp file.
                    // Store the hint so sendFileViaRelay can resume from the right offset when
                    // the user re-initiates the transfer (or the auto-retry kicks in).
                    const { transferId, resumeOffset } = message;
                    if (typeof transferId !== 'string' || typeof resumeOffset !== 'number') break;
                    if (resumeOffset < 0 || !Number.isFinite(resumeOffset)) break;

                    console.log(`[Relay] Reconnect hint for ${transferId}: desktop claims ${resumeOffset} bytes on disk`);
                    this._pendingResumeHints = this._pendingResumeHints || new Map();
                    this._pendingResumeHints.set(transferId, resumeOffset);
                    break;
                }
  ...
  async createConnection(targetPeerId) {
    ...
    console.log(
      ...
      this.rtcConfig.iceServers.length,
    );
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        console.log(
          `[WebRTC] ICE candidate: ${type} (${event.candidate.protocol || "unknown"})`,
        );
        ...
        console.log(
          "[WebRTC] ICE gathering complete. Candidates:",
          candidateCount,
        );
      }
    };
    ...
    pc.oniceconnectionstatechange = () => {
      ...
      if (pc.iceConnectionState === "failed") {
        console.error(
          "[WebRTC] ICE connection failed! Attempting ICE restart. Candidates gathered:",
          candidateCount,
        );
        this.performIceRestart(pc, targetPeerId).catch((err) => {
          console.error("[WebRTC] ICE restart failed:", err);
          this.handleConnectionFailure(targetPeerId, "ICE restart failed");
        });
      }
    };
  ...
  async performIceRestart(pc, targetPeerId) {
    if (!pc || pc.signalingState === "closed") return;
    console.log(`[WebRTC] Performing ICE restart for peer ${targetPeerId}`);
    try {
      ...
      const signedAnnouncement = await this.signPeerAnnouncement(announcement);
      this.ws.send(JSON.stringify(signedAnnouncement));
      console.log(`[WebRTC] ICE restart offer sent to ${targetPeerId}`);
    } catch (err) {
      console.error(`[WebRTC] ICE restart error for ${targetPeerId}:`, err);
      throw err;
    }
  }
  ...
  async handleOffer(fromPeerId, sdp, fingerprint, publicKey) {
    try {
      ...
      // ===== PHASE 0: FINGERPRINT VALIDATION (BULLETPROOF) =====
      if (fingerprint) {
        const validation = await this.validatePeerFingerprint(
          fromPeerId,
          fingerprint,
        ...
        // BULLETPROOF: Don't reject on first contact - flag for verification instead
        if (!validation.valid) {
          console.warn(
            `[Security] Fingerprint mismatch for ${fromPeerId}: ${validation.reason}`,
          );

          // MITM detection - only reject if we have a TRUSTED record that doesn't match
          const trusted = this.trustedDevices?.get
            ? this.trustedDevices.get(fromPeerId)
            : null;
          const hasTrustedRecord = trusted && trusted.trustLevel === "trusted";

          if (hasTrustedRecord) {
            // This is a MITM attack - we have a trusted record and it doesn't match
            console.error(
              `[Security] MITM DETECTED! Rejecting peer ${fromPeerId}`,
            );
            this.handleConnectionFailure(
              fromPeerId,
              "MITM detected - fingerprint mismatch",
            );
          ...
          // First-time peer or pending verification - allow connection but flag it
          console.log(
            `[Security] New/pending peer ${fromPeerId} - allowing connection with verification flag`,
          );
          this.pendingVerification = this.pendingVerification || new Map();
          this.pendingVerification.set(fromPeerId, {
            fingerprint,
            reason: validation.reason,
            timestamp: Date.now(),
          });

          // Notify UI for manual verification
          if (this.onPeerVerificationNeeded) {
            this.onPeerVerificationNeeded(fromPeerId, fingerprint, validation);
          }
        } else {
          // Valid fingerprint - trusted or auto-verified
          console.log(
            `[Security] Peer ${fromPeerId} fingerprint valid (${validation.trustLevel})`,
          );
        ...
        console.warn(
          `[Security] Peer ${fromPeerId} sent no fingerprint - allowing but flagging`,
        );
      }
      ...
      // A second offer from the same peer means the old connection is stale.
      ...
      pc.onicecandidate = (event) => {
        if (event.candidate) {
          ...
          candidateCount[type] = (candidateCount[type] || 0) + 1;
          console.log(
            `[WebRTC-Recv] ICE candidate: ${type} (${event.candidate.protocol || "unknown"})`,
          );
          ...
          console.log(
            "[WebRTC-Recv] ICE gathering complete. Candidates:",
            candidateCount,
          );
        }
      };
      ...
      pc.oniceconnectionstatechange = () => {
        console.log(
          "[WebRTC-Recv] ICE connection state:",
      ...
      pc.onconnectionstatechange = () => {
        ...
        if (
          ...
          console.error(
            "[WebRTC-Recv] Connection failed! Candidates:",
            candidateCount,
          );
  ...
  setupDataChannel(dc, peerId) {
    ...
    dc.onerror = (error) => {
      ...
      this.handleError(ErrorCodes.DATACHANNEL_ERROR, {
        peerId,
        error: error.message,
      });
    };
  ...
  async handleDataChannelMessage(peerId, data) {
    ...
    if (typeof data === "string") {
      ...
      try {
        ...
        console.warn(
          "[DataChannel] Dropping non-JSON control message:",
          e.message,
        );
        return;
      }
      ...
      if (msg.type === "file-start") {
        ...
        try {
          ...
          console.error(
            "[FileTransfer] Rejected file-start:",
        ...
        if (msg.relativePath) {
          try {
            ...
            console.warn(
              "[FileTransfer] Relative path rejected, using filename only:",
              e.message,
            );
        ...
        if (typeof msg.sha256 !== "undefined" && msg.sha256 !== null && !expectedSha256) {
          console.error(
            "[FileTransfer] Rejected file-start: invalid SHA-256 format",
          );
          const dc = this.dataChannels.get(peerId);
          if (dc?.readyState === "open") {
            dc.send(
        ...
        if (
          ...
          if (resumeState.resumeCapable === true) {
            ...
            console.log(
              `[Resume] Safe resume approved for ${safeFilename}: ${resumeOffset}/${msg.size}`,
            );
          } else {
            console.log(
              `[Resume] Restarting ${safeFilename} from byte 0 (partial state is not durably resumable)`,
            );
          }
        }
        ...
        const transferState = {
            ...
            pausedChunks: [], // Queue chunks received while paused
        };

        // For large files, try to use File System Access API
        // Uses resolveFileHandle: checks pre-stored handle (from user gesture),
        // falls back to showSaveFilePicker (works only in user gesture context).
        if (transferState.useStreaming && this.useFileSystemAPI) {
          try {
            const handle = await this.resolveFileHandle(
              transferState.metadata.filename,
              transferState.metadata.mimeType,
            );
            if (handle) {
              transferState.fileHandle = handle;
              const writable = await transferState.fileHandle.createWritable();
              transferState.writer = writable;
              console.log(
                "[FileTransfer] Using File System Access API for streaming",
              );
            } else {
              console.log(
                "[FileTransfer] No file handle available, using memory buffer",
              );
            ...
            console.log(
              "[FileTransfer] File picker cancelled/unavailable, using memory buffer",
            );
            transferState.useStreaming = false;
          }
          ...
          if (window.streamSaver) {
            try {
              ...
              console.log(
                "[FileTransfer] StreamSaver.js failed, using memory buffer",
              );
              transferState.useStreaming = false;
            }
          } else {
            console.log(
              "[FileTransfer] No streaming API available, using memory buffer (file may be too large)",
            );
            transferState.useStreaming = false;
          }
        }
        ...
        if (
          !transferState.useStreaming &&
          msg.size > this.MAX_IN_MEMORY_RECEIVE_SIZE
        ) {
          const dc = this.dataChannels.get(peerId);
          if (dc?.readyState === "open") {
            dc.send(
        ...
        // Receiver-side timeout: cancel if no chunks arrive within TRANSFER_TIMEOUT
        this.setTransferTimeout(msg.transferId);
        ...
        this.assembleFile(msg.transferId, msg.sha256);
        ...
        if (state) {
          const offset = Number.isFinite(msg.resumeOffset)
            ? Math.max(0, msg.resumeOffset)
            : 0;
          state.resumeOffset = offset;
          state.resumeReady = true;
          state.resumeCapable = !!msg.resumeCapable;
          console.log(
            `[Resume] Sender: resume-ready offset ${offset} for ${msg.transferId}`,
          );
        }
        ...
        const state = this.activeTransfers.get(msg.transferId);
        if (state) {
          state.resumeOffset = Number.isFinite(msg.resumeOffset)
            ? Math.max(0, msg.resumeOffset)
            : 0;
          state.resumeReady = true;
          state.resumeCapable = true; // legacy peers only send this signal when they intend resume support
          console.log(
            `[Resume] Sender: receiver requested offset ${state.resumeOffset} for ${msg.transferId}`,
          );
        }
        ...
        console.warn(
          `[FileTransfer] Remote cancelled transfer ${msg.transferId}: ${msg.reason || "rejected"}`,
        );
      ...
      if (transfer && state && !state.cancelled) {
        // Buffer chunks when paused; process immediately when active
        if (state.paused) {
          transfer.pausedChunks.push(chunkData);
          return;
        }

        // Flush any queued paused chunks first
        if (transfer.pausedChunks.length > 0) {
          const queued = transfer.pausedChunks.splice(0);
          for (const qd of queued) {
            await this._writeTransferChunk(transfer, qd);
          }
        }

        await this._writeTransferChunk(transfer, chunkData);
        state.bytesTransferred = transfer.received;

        // Receiver-side: reset timeout on each received chunk
        ...
        if (
          Number.isFinite(transfer.metadata.size) &&
          ...
          if (this.onTransferError) {
            this.onTransferError({
              ...
              error:
                "Received more bytes than declared file size. Transfer aborted.",
            });
          }
          return;
        }
  ...
  async assembleFile(transferId, senderSha256) {
    const transfer = this.incomingChunks.get(transferId);
    const state = this.activeTransfers.get(transferId);

    // Receiver-side: clear timeout on completion
    this.clearTransferTimeout(transferId);
    ...
    const expectedSize = Number.isFinite(transfer.metadata?.size)
      ? transfer.metadata.size
      : null;
    // Use sha256 from file-end message if metadata didn't have it (incremental hash path).
    const expectedSha256 = this.normalizeSha256(transfer.metadata?.sha256) || this.normalizeSha256(senderSha256);
    const actualSha256 = transfer.hasher ? await transfer.hasher.hex() : null;
    ...
    const getIntegrityError = () => {
      ...
      if (expectedSha256 && actualSha256 !== expectedSha256) {
        ...
        const actualShort = actualSha256
    ...
    if (transfer.useStreaming && transfer.writer) {
      ...
      try {
        ...
        await failTransfer(
          `Could not finalize streamed file: ${e.message || e}`,
        );
        return;
      }
    } else {
      // Memory buffer approach - assemble and download
      totalSize = transfer.chunks.reduce(
    ...
    this.showNotification(
      "Transfer Complete",
      `Received: ${transfer.metadata.filename}`,
    );
  ...
  cancelTransfer(transferId) {
    ...
    // Notify remote peer via DataChannel (P2P path)
    ...
    // Notify remote peer via signaling (relay path)
    if (state.isRelay && state.peerId && this.ws?.readyState === WebSocket.OPEN) {
      this.ws.send(
        JSON.stringify({
          type: "relay-cancel",
          to: state.peerId,
    ...
    // Clean up resources (timers, writers, chunks)
    this.cleanupTransfer(transferId);

    // Broadcast cancel to other tabs
    this.broadcastEvent("transfer-cancel", {
      transferId,
      peerId: state.peerId,
    ...
    if (this.onTransferError) {
      this.onTransferError({
        transferId,
        filename: state.filename || state.fileIndex != null ? `File ${(state.fileIndex || 0) + 1}` : undefined,
        error: "Transfer cancelled manually.",
      });
    }
  }
    ...
    const reasonText =
      {
        ...
        "invalid-sha256":
          "Receiver rejected file integrity metadata (invalid SHA-256).",
        "receiver-memory-limit":
          "Receiver cannot store this file size in memory on the current browser.",
        "size-overflow":
          "Receiver detected byte overflow and aborted transfer.",
  ...
  handleTransferPause(transferId, paused) {
    ...
    // When resuming, flush any chunks that were buffered during pause
    if (!paused) {
      const transfer = this.incomingChunks.get(transferId);
      if (transfer && transfer.pausedChunks && transfer.pausedChunks.length > 0) {
        const queued = transfer.pausedChunks.splice(0);
        (async () => {
          for (const qd of queued) {
            await this._writeTransferChunk(transfer, qd);
          ...
          state.bytesTransferred = transfer.received;
          if (this.onTransferProgress && Number.isFinite(transfer.metadata?.size)) {
            this.onTransferProgress({
              transferId,
              filename: transfer.metadata.filename,
              received: transfer.received,
  ...
  async _writeTransferChunk(transfer, chunkData) {
    if (transfer.useStreaming && transfer.writer) {
      try {
  ...
  /**
   * Store a FileSystemFileHandle obtained during a user gesture (click/tap).
   * The handle is consumed later by async transfer code that cannot trigger
   * showSaveFilePicker() itself (requires user gesture).
   *
   * @param {string} filename – suggested file name (used as lookup key)
   * @param {FileSystemFileHandle} handle – from showSaveFilePicker()
   */
  setFileSaveHandle(filename, handle) {
    if (handle && typeof handle.createWritable === "function") {
      this._pendingFileHandles.set(filename, handle);
    }
  }

  /**
   * Attempt to resolve a FileSystemFileHandle for `filename`.
   * Returns a pre-stored handle (from setFileSaveHandle) if available,
   * otherwise calls showSaveFilePicker() directly (may fail outside user
   * gesture contexts).
   *
   ...
   */
  async resolveFileHandle(filename, mimeType) {
    // Check for a pre-stored handle first
    if (this._pendingFileHandles.has(filename)) {
      const handle = this._pendingFileHandles.get(filename);
      this._pendingFileHandles.delete(filename);
      return handle;
    }

    // Fallback: try showSaveFilePicker (works only in user-gesture context)
  ...
  getRelayReasonMessage(reason) {
    return (
      {
        "sha256-mismatch":
          "Relay receiver rejected file due to SHA-256 mismatch.",
        ...
        "receiver-memory-limit":
          "Relay receiver cannot store this file size in memory on the current browser.",
      }[reason] || null
    );
  }
  ...
  async requestFileSend(targetPeerId, files) {
    ...
    if (totalBytes > 100 * 1024 * 1024 * 1024) {
      throw new Error(
        `Total size exceeds limit: ${this.formatSize(totalBytes)} (max 100 GB)`,
      );
    }
    // ---- End security guards ----
    ...
    if (isRelayPeer2) {
      ...
      return new Promise((resolve, reject) => {
        // Register a per-peer handler in the Map (no monkey-patching)
        const timeoutId = setTimeout(() => {
          this._fileResponseHandlers.delete(targetPeerId);
          reject(new Error("Desktop peer did not respond to file request (timeout)"));
        }, 15000);

        this._fileResponseHandlers.set(targetPeerId, (accepted, fromId) => {
          clearTimeout(timeoutId);
          if (!accepted) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const check = () => {
        ...
        if (Date.now() - startTime > timeout) {
          ...
          reject(
            new Error(
              "DataChannel timeout - peer may be behind NAT or firewall",
            ),
          );
          return;
        }
  ...
  waitForResumeReady(transferId, timeout = this.RESUME_READY_TIMEOUT) {
    return new Promise((resolve, reject) => {
      ...
      const check = () => {
        ...
        if (Date.now() - startTime > timeout) {
          console.warn(
            `[Resume] No explicit resume-ready for ${transferId} after ${timeout}ms; restarting from 0`,
          );
          state.resumeReady = true;
  ...
  waitForRelayVerification(
    transferId,
    timeout = this.RELAY_VERIFICATION_TIMEOUT,
  ) {
    return new Promise((resolve, reject) => {
      const startTime = Date.now();
      ...
      const check = () => {
        ...
        if (!state) {
          reject(
            new Error("Relay transfer state missing during verification wait"),
          );
          return;
        }
        ...
        if (state.cancelled) {
          reject(
            new Error(
              state.cancelReason ||
                "Relay transfer cancelled before verification",
            ),
          );
          return;
        }
  ...
  handleFileResponse(fromPeerId, accepted) {
    console.log("[FileTransfer] handleFileResponse called:", {
      fromPeerId,
      accepted,
    });

    // Check if there's a per-peer handler registered (for relay path)
    const customHandler = this._fileResponseHandlers.get(fromPeerId);
    if (customHandler) {
      this._fileResponseHandlers.delete(fromPeerId);
      customHandler(accepted, fromPeerId);
      return;
    }
    ...
    if (!pending) {
      console.log(
        "[FileTransfer] No pending files found for peer:",
        fromPeerId,
      );
      return;
    }
    ...
    if (accepted) {
      console.log(
        "[FileTransfer] Transfer accepted, establishing P2P pipeline...",
      );
      ...
      (async () => {
        try {
          ...
          if (
            !this.dataChannels.has(fromPeerId) ||
            this.dataChannels.get(fromPeerId).readyState !== "open"
          ) {
            if (!this.peers.has(fromPeerId)) {
          ...
          console.log(
            "[FileTransfer] sendFiles completed successfully via P2P",
          );
          pending.resolve();
        } catch (err) {
          ...
          if (this.useRelayFallback) {
            ...
            try {
              ...
              console.log(
                "[FileTransfer] Relay transfer completed successfully",
              );
              pending.resolve();
            } catch (relayErr) {
  ...
  acceptFileRequest(fromPeerId) {
    this.ws.send(
      JSON.stringify({ type: "file-response", to: fromPeerId, accepted: true }),
    );
  }
  ...
  async sendFiles(targetPeerId, files) {
    ...
    console.log(
      "[FileTransfer] DataChannel state:",
      dc?.readyState,
      "for peer:",
      targetPeerId,
    );
    ...
    if (!dc) {
      console.error(
        "[FileTransfer] No DataChannel found for peer:",
        targetPeerId,
      );
    ...
    if (dc.readyState !== "open") {
      console.error(
        "[FileTransfer] DataChannel not open, state:",
        dc.readyState,
      );
  ...
  async sendFileViaRelay(
    ...
    // Check if the desktop signaled a resume offset for this (name, size) pair.
    // We match by transferId if forced (reconnect path), or by querying the
    // hint map keyed by transferId that the desktop sent via relay-reconnect-hint.
    const transferId = forcedTransferId || this.generateTransferId();

    // Look for a pending resume hint for this transferId
    let startOffset = 0;
    if (this._pendingResumeHints?.has(transferId)) {
      startOffset = this._pendingResumeHints.get(transferId);
      this._pendingResumeHints.delete(transferId);
      // Clamp to valid range
      if (startOffset < 0 || startOffset >= file.size) startOffset = 0;
      if (startOffset > 0) {
        console.log(
          `[Relay] Resuming ${file.name} from byte ${startOffset} (of ${file.size})`,
        );
      }
    }

    const startTime = Date.now();
    // Compute SHA-256 incrementally during transfer instead of reading the
    // entire file upfront.
    const streamHasher = new IncrementalSHA256();
    ...
    this.activeTransfers.set(transferId, {
      ...
      relayVerifyOk: false,
      relayVerifyReason: "",
      sha256: null, // Computed incrementally; sent in relay-end
    });

    // Send start message (sha256 sent as null — final hash arrives in relay-end)
    this.ws.send(
      JSON.stringify({
        type: "relay-start",
        to: targetPeerId,
        ...
        fileIndex,
        totalFiles,
        encrypted: this.sharedSecrets.has(targetPeerId) && this.encryptionEnabled,
      }),
    );
    ...
    const fastBase64 = (/** @type {Uint8Array} */ u8) => {
      ...
      for (let b = 0; b < len; b++) parts[b] = String.fromCharCode(u8[b]);
      return btoa(parts.join(""));
    };

    // Slice the file from startOffset so we don't re-send already-received data.
    // File.slice() returns a Blob; .stream().getReader() gives us a streaming reader.
    const fileSlice = startOffset > 0 ? file.slice(startOffset) : file;
    const reader = fileSlice.stream().getReader();
    let offset = startOffset; // byte position relative to start of full file
    /** @type {Promise<void>[]} */
    let pipeline = [];
    ...
    const sendChunk = async (chunk, chunkOffset) => {
      ...
      // Backpressure: wait if WS send buffer is congested (>1 MB pending)
      const BACKPRESSURE_HIGH = 1024 * 1024;
      const BACKPRESSURE_LOW = 256 * 1024;
      while (this.ws.bufferedAmount > BACKPRESSURE_HIGH) {
        await new Promise((r) => setTimeout(r, 5));
        if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
          throw new Error("WebSocket disconnected during backpressure wait");
        }
      }
      let payload;
      if (targetPeerId && this.sharedSecrets.has(targetPeerId) && this.encryptionEnabled) {
        ...
        if (encrypted) {
          payload = {
            ...
            to: targetPeerId,
            transferId,
            data: fastBase64(new Uint8Array(encData)),
            iv: fastBase64(iv),
            encrypted: true,
            offset: chunkOffset,
          };
    ...
    try {
      while (true) {
        ...
        // Wait out pause with escape: check cancellation and enforce a
        // maximum pause duration (10 min) so the loop never hangs forever.
        if (state?.paused) {
          ...
          while (state.paused) {
            ...
            if (Date.now() - pauseStart > MAX_PAUSE_MS) {
              console.warn(`[Relay] Pause timeout for ${transferId} — cancelling`);
        ...
        for (let i = 0; i < value.length; i += RELAY_CHUNK_SIZE) {
          ...
          // Feed chunk to the streaming hasher (non-blocking — just stores ref).
          streamHasher.update(chunk);

          // Live skip: if the desktop sent a relay-resume-request mid-flight
          // (brief WS blip during active transfer), discard chunks it already has.
          const liveState = this.activeTransfers.get(transferId);
          if (
            liveState?.resumeRequested &&
            chunkOffset < liveState.resumeOffset
          ) {
            offset += chunk.length;
            continue; // desktop already has this chunk
          }
          if (
            liveState?.resumeRequested &&
            chunkOffset >= liveState.resumeOffset
          ) {
            // Caught up — clear the flag
            liveState.resumeRequested = false;
          }
          ...
          // Update progress
          // Reset transfer timeout on progress — slow but active transfers
          // must not be killed.
          this.setTransferTimeout(transferId);
      ...
      // Finalise the incremental hash and include it in relay-end so the
      // receiver can verify integrity.
      const fileSha256 = await streamHasher.hex();
      this.ws.send(
        JSON.stringify({
          type: "relay-end",
          to: targetPeerId,
      ...
      if (verification.supported && !verification.ok) {
        throw new Error(
          this.getRelayReasonMessage(verification.reason) ||
            verification.reason ||
            "Relay receiver integrity verification failed",
        );
      }
      ...
      // Compute partial hash from whatever chunks were fed before the error.
      const fileSha256 = await streamHasher.hex().catch(() => null);
      this.activeTransfers.delete(transferId);
  ...
  async sendFile(dc, file, fileIndex = 0, totalFiles = 1, targetPeerId = null) {
    ...
    // Compute SHA-256 incrementally during transfer instead of reading the
    // entire file upfront.  This avoids blocking transfer start for large files.
    const streamHasher = new IncrementalSHA256();
    ...
    console.log(
      `[FileTransfer] Using chunk size ${chunkSize / 1024}KB for ${file.size / ((1024 * 1024) | 0)}MB file`,
    );
    ...
    this.activeTransfers.set(transferId, {
      ...
      resumeReady: false,
      resumeCapable: false,
      sha256: null, // Computed incrementally; sent in file-end
    ...
    try {
      // sha256 is null here — the receiver will get the final hash in file-end.
      dc.send(
        JSON.stringify({
          type: "file-start",
          transferId,
      ...
      if (skipBytes > 0) {
        console.log(
          `[Resume] Sender: skipping first ${skipBytes} bytes of ${file.name}`,
        );
      }
      ...
      while (true) {
        ...
        if (state?.paused) {
          ...
          while (state.paused) {
            ...
            if (state.cancelled) throw new Error("Cancelled");
            if (Date.now() - pauseStart > MAX_PAUSE_MS) {
              console.warn(`[FileTransfer] Pause timeout for ${transferId} — cancelling`);
              this.cancelTransfer(transferId);
              throw new Error("Pause timeout");
            }
          }
      ...
      // Finalise the incremental hash and include it in file-end so the
      // receiver can verify integrity.
      const fileSha256 = await streamHasher.hex();
      dc.send(JSON.stringify({ type: "file-end", transferId, sha256: fileSha256 }));

      const verification = await this.waitForFileVerification(transferId);
      if (verification.supported && !verification.ok) {
        throw new Error(
          verification.reason || "Receiver integrity verification failed",
        );
      }
      ...
      if (parsed.server && parsed.server !== this.serverUrl) {
        ...
        if (typeof parsed.server === "string" && parsed.server.trim()) {
          ...
          try {
            ...
            const hostPort = rawServer.replace(/^\w+:\/\//, "").split("/")[0];
  ...
  // cleanupPeer is defined once above (line ~550) with full resource cleanup logic.
  // The duplicate stub that was here has been removed to prevent it from overriding
  // the comprehensive implementation.

  getTheme() {
    return this.loadSetting("teleport-theme") || "dark";
  }
  setTheme(theme) {
    this.saveSetting("teleport-theme", theme);
  }
  ...
  setTransferTimeout(transferId, timeoutMs = this.TRANSFER_TIMEOUT) {
    ...
    this.resumeStateExpiry.set(
      transferId,
      Date.now() + this.RESUME_STATE_LIFETIME,
    );
  }
  ...
  generateTransferId() {
    ...
    while (pseudoHex.length < 32) {
      pseudoHex += Math.floor(Math.random() * 0x100000000)
        .toString(16)
        .padStart(8, "0");
    }
...
// ==================== INCREMENTAL SHA-256 (duplicate removed) ====================
// The IncrementalSHA256 class is defined once at the top of this file (line ~150).
// This duplicate definition has been removed to prevent the second class from
// overriding the first, which caused "Cannot update SHA-256 after digest()" errors.
...
class QRCodeGenerator {
  ...
  getAlignmentPositions() {
    ...
    const step =
      this.version === 32
        ? 26
        : Math.ceil((this.size - 13) / (count - 1) / 2) * 2;
    for (let i = count - 1; i >= 1; i--)
      positions.unshift(this.size - 7 - (count - 1 - i) * step);
    return positions;
  }
  ...
  reserveFormatArea() {
    ...
    for (let i = 0; i < 8; i++) {
      if (this.modules[8][this.size - 1 - i] === null)
        this.modules[8][this.size - 1 - i] = false;
      if (this.modules[this.size - 1 - i][8] === null)
  ...
  encodeData() {
    ...
    while (bits.length < totalCap * 8)
      bits += bits.length % 16 < 8 ? "11101100" : "00010001";
    const result = [];
  ...
  generateECC(data) {
    const ecCount =
      QRCodeGenerator.EC_CODEWORDS[this.ecLevel][this.version - 1];
  ...
  getGeneratorPolynomial(degree) {
    ...
    for (let i = 0; i < degree; i++) {
      ...
      for (let j = 0; j < poly.length; j++) {
        next[j] ^= QRCodeGenerator.GF256.mul(
          poly[j],
          QRCodeGenerator.GF256.exp[i],
        );
        next[j + 1] ^= poly[j];
      }
      poly = next;
  ...
  evaluatePenalty() {
    ...
    for (let r = 0; r < this.size; r++) {
      for (let c = 0; c < this.size - 4; c++) {
        if (
          this.modules[r].slice(c, c + 5).every((m) => m === this.modules[r][c])
        )
          penalty += 3;
      }
    }
...
// Post-class static property assignment (compatible with all browsers,
// avoids ES2022 static class field syntax that crashes Firefox < 90)
QRCodeGenerator.EC_CODEWORDS = {
